...
target_dir = '/Users/diegorivero/.gemini/antigravity/scratch/ecuador-invest'
...
def replace_in_file(filepath):
    ...
    # Brand replacements
    content = content.replace('Pilar Patrimonio', 'Ecuador Invest')
    content = content.replace('PILAR PATRIMONIO', 'ECUADOR INVEST')
    content = content.replace('pilar-patrimonio', 'ecuador-invest')

    # Footer replacement
    # We want to remove the footer-contact-grid and replace it with a centered Ecuador one.
    ...
    new_footer = """<div class="footer-contact-grid" style="grid-template-columns: 1fr; justify-items: center; max-width: 400px; margin: 0 auto;">
                <!-- Ecuador -->
                <a href="tel:+59327085340" class="contact-card" style="width: 100%; max-width: 300px;">
    ...
    # Form Default Selection: Remove 'selected' from any option, add to Ecuador (+593)
    content = content.replace('selected', '')
    content = content.replace('<option value="+593">🇪🇨 +593</option>', '<option value="+593" selected>🇪🇨 +593</option>')
